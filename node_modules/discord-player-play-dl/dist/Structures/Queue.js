...
class Queue {
        ...
        this.connection.on("finish", async (resource) => {
            ...
            if (!this.tracks.length && this.repeatMode === types_1.QueueRepeatMode.OFF) {
                ...
                if (this.repeatMode === types_1.QueueRepeatMode.TRACK)
                    return void this.play(this.current, { immediate: true });
    ...
    async play(src, options = {}) {
        ...
        if (["youtube", "spotify", "soundcloud"].includes(track.raw.source)) {
            ...
            if (track.raw.source === "spotify" && !track.raw.engine) {
                track.raw.engine = await play_dl_1.default
    ...
    async _handleAutoplay(track) {
        ...
        const info = await play_dl_1.default.video_info(track.url).catch(Util_1.Util.noop);
        if (!info) {
            if (this.options.leaveOnEnd)
                this.destroy();
            return void this.player.emit("queueEnd", this);
        }
        const related = await play_dl_1.default.video_info(info.related_videos[Math.floor(Math.random() * (info.related_videos.length - 1))]);
        const nextTrack = new Track_1.default(this.player, {
            title: related.video_details.title,
            url: related.video_details.url,
            duration: related.video_details.durationRaw ? Util_1.Util.buildTimeCode(Util_1.Util.parseMS(related.video_details.durationInSec * 1000)) : "0:00",
            description: "",
            thumbnail: Util_1.Util.last(related.video_details.thumbnails).url,
